...
const search2DArray = (twoDimensionalArray, row, col, word) => {
  ...
  for (let dir = 0; dir < 8; dir++) {
    ...
    for (charToCheck = 1; charToCheck < word.length; charToCheck++) {
      if (currX >= twoDimensionalArray.length || currX < 0 || currY >= twoDimensionalArray[0].length || currY < 0) break;

      if (twoDimensionalArray[currX][currY] !== word[charToCheck]) break;

      currX += xDir[dir];
      currY += yDir[dir];
    }

    if (charToCheck === word.length) return true;
  }
...
function searchForWord(twoDimensionalArray, word) {
  ...
  for (let i = 0; i < twoDimensionalArray.length; i++) {
    for (let j = 0; j < twoDimensionalArray[0].length; j++) {
      if (search2DArray(twoDimensionalArray, i, j, word)) {
        count++;
      }
    }
  }
